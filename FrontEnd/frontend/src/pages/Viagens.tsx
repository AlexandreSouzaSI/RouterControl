...
export function Viagens() {
    ...
    async function excluirPeriodo(caminhaoId: string, mes: string, placa: string) {
        if (!caminhaoId) {
            alert(
                'Esse registro é antigo e não tem o caminhão vinculado — atualize a página e tente de novo.',
            );
            return;
        }

        const confirmar = confirm(
            `Excluir os dados de ${placa} em ${mes}? Isso apaga viagens e paradas desse mês. Não tem como desfazer — pra voltar, suba o arquivo de novo.`,
        );

        if (!confirmar) return;

        try {
            await api.delete(`/caminhoes/${caminhaoId}/periodos/${mes}`);
    ...
    const seriesPorCaminhao = useMemo(() => {
        if (!data?.dadosPorMes) return [];

        const placas = placaFiltro
            ? [placaFiltro]
            : Array.from(
                new Set(
                    mesesOrdenados.flatMap((mes: string) =>
                        (data.dadosPorMes[mes]?.ranking || []).map(
                            (r: any) => r.placa,
                        ),
                    ),
                ),
        ...
        return placas.map((placa) => ({
            placa,
            valores: mesesOrdenados.map((mes: string) => {
                const item = (
                    data.dadosPorMes[mes]?.ranking || []
                ).find((r: any) => r.placa === placa);

                return item?.totalViagens ?? 0;
            }),
        }));
                            ...
                            >
                                ...
                                <td className="text-right py-2">
                                    <button
                                        ...
                                        title="Excluir dados desse mês"
                                        className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 dark:hover:text-red-400 transition"
                                    >
                                        <Trash2 size={15} />
                                    </button>
                                </td>
